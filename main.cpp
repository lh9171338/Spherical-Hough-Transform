...
{
	/******************* Parameters *******************/
	string srcImageFilename = "../image/src.png";
	string dstImageFilename = "../image/dst.png";
...
//	 temp(Rect(0, 0, cols / 2, rows)).copyTo(srcImage(Rect(cols * 3 / 2, 0, cols / 2, rows)));
	 ...
	 /******************* Detect lines *******************/
	 vector<Vec4f> lines;
	 sphericalcamera::HoughLinesProbabilistic(points, lines, 0.5f, 0.5f, 50, 5, 10, 300);
	 ...
	 pointClouds.clear();
	 sphericalcamera::drawLines(pointClouds, lines, 0.1f, Scalar(0, 0, 255));
	 sphericalcamera::ReProjectSphericalImage(dstImage, pointClouds, sphericalIntrinsic, FOV, Scalar(0, 0, 255), 2);
	 if (saveDstImageFlag)
