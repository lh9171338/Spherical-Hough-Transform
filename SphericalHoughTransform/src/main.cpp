...
{
	/******************* Parameters *******************/
	string srcImageFilename = "image/fisheye.png";
	string dstImageFilename = "image/fisheye-result.png";
	String modelFilename = "param/model.yml";
	string intrinsicFilename = "param/fisheye.yaml";
	string pointclondFilename = "dst.ply";
	string outputFilename = "arcs.txt";
	bool saveImagePointcloudFlag = false;
	bool saveEdgePointcloudFlag = false;
	bool saveLinePointcloudFlag = false;
	...
	sphericalcamera::FisheyeIntrinsic fisheyeIntrinsic;
	FileStorage fs(intrinsicFilename, FileStorage::READ);
	if (!fs.isOpened())
		cerr << "Open intrinsic file failed!" << endl;
	fs["K"] >> fisheyeIntrinsic.cameraMatrix;
	fs["D"] >> fisheyeIntrinsic.distCoeffs;

	/******************* Read image *******************/
	Mat srcImage = imread(srcImageFilename);
	if (srcImage.empty())
		cerr << "Read image file failed!" << endl;
	...
	sphericalcamera::ProjectFisheyeEdge(edge, points, fisheyeIntrinsic);
	cout << "Number of edge points: " << points.size() << endl;

	/******************* Detect line segments *******************/
	vector<Vec4f> lines;
	sphericalcamera::HoughLinesProbabilistic(points, lines, 0.5f, 0.5f, 50, 5, 5, 300);
	...
	if (saveImagePointcloudFlag)
		sphericalcamera::ProjectFisheyeImage(srcImage, pointClouds, fisheyeIntrinsic);
	...
	pointClouds.clear();
	sphericalcamera::drawLines(pointClouds, lines, 0.1f, Scalar(0, 0, 255));
	sphericalcamera::ReProjectFisheyeImage(dstImage, pointClouds, fisheyeIntrinsic, true, Scalar(0, 0, 255), 5);
	if (saveDstImageFlag)
...
{
	/******************* Parameters *******************/
	string srcImageFilename = "image/spherical.png";
	string dstImageFilename = "image/spherical-result.png";
	String modelFilename = "param/model.yml";
	string intrinsicFilename0 = "param/spherical.yaml";
	string intrinsicFilename1 = "param/spherical.yaml";
